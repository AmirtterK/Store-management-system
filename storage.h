    ...
    for (i = 0; i < n; i++)
    {
        ...
        do
        {
            ...
            for (j = 22 + number_of_digits(car[i + item_in_line].price); j <= 65 - (number_of_digits(car[i + item_in_line].mileage) + number_of_digits(car[i + item_in_line].chassis.year) + 8); j++)
                printf(" ");
            printf("%d | %d km  |", car[i + item_in_line].chassis.year, car[i + item_in_line].mileage);
        } while (item_in_line++ < 2 && i + item_in_line < n);
        printf("\n");
        item_in_line = 0;
        do
...
int display(specs car)
{
    ...
    gotoxy(14, 18);
    printf("MAXIMUM POWER: %d HORSEPOWER", car.motor.hp);
    gotoxy(14, 19);
    printf("MAXIMUM TORQUE: %d NM", car.motor.torque);
    gotoxy(14, 20);
    printf("MAXIMUM SPEED: %.1f KM/H", car.top_speed);
    gotoxy(14, 21);
    printf("0-100 KM/H (62MPH) IN: %.1f s", car.zero_to_one_undred_time);
    gotoxy(14, 22);
    printf("ENGINE DISPLACEMENT: %d CC", car.motor.Displacement);
    gotoxy(14, 23);
    printf("OVERALL WEIGHT: %d KG", car.chassis.weight);
    gotoxy(14, 24);
    printf("OVERALL WIDTH: %.3f M", car.chassis.width);
    gotoxy(14, 25);
    printf("OVERALL LENGTH: %.3f M", car.chassis.length);
    gotoxy(14, 26);
    printf("OVERALL HEIGHT: %.3f M", car.chassis.height);
    gotoxy(14, 27);
    printf("OVERALL WHEELBASE: %.3f M", car.chassis.wheelBase);
    gotoxy(150, 15);
    printf("YEAR                   %d", car.chassis.year);
    gotoxy(150, 17);
    printf("MILAEGE                %d KM", car.mileage);
    gotoxy(150, 19);
    printf("EXTERIOR COLOR         %s", car.chassis.exterior_color);
    gotoxy(150, 21);
    printf("INTERIOR COLOR         %s", car.chassis.interior_color);
    gotoxy(150, 23);
    printf("DRIVE TYPE             %s", car.chassis.drive_type);
    gotoxy(150, 25);
    printf("ENGINE                 %.1fL %s%d %s", car.motor.liters, car.motor.shape, car.motor.cylinders, car.motor.aspiration);
    gotoxy(173, 26);
    printf("ENGINE");
    ...
    gotoxy(150, 28);
    printf("BODY TYPE              %s", car.chassis.Body_type);
    gotoxy(150, 30);
    printf("TRANSMISSION           %s", car.chassis.transmission);
    gotoxy(173, 31);
    printf("TRANSMISION");
    gotoxy(168, 39);
    printf("%d EURO", car.price);
